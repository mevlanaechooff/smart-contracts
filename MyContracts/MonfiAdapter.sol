...
abstract contract MonfiAdapter is Maintainable {
    using SafeERC20 for IERC20;

    event MonfiAdapterSwap(
    ...
    function recoverERC20(
        ...
        require(_tokenAmount > 0, "MonfiAdapter: Nothing to recover");
        IERC20(_tokenAddress).safeTransfer(msg.sender, _tokenAmount);
        emit Recovered(_tokenAddress, _tokenAmount);
    }

    function recoverNAD(uint256 _amount) external onlyMaintainer {
        require(_amount > 0, "MonfiAdapter: Nothing to recover");
    ...
    function swap(
        ...
        require(diff >= _amountOutReceived, "Insufficient amount-out");
        emit MonfiAdapterSwap(_fromToken, _toToken, _amountIn, _amountOut);
    }
