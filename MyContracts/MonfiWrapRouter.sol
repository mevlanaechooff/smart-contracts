...
contract MonfiWrapRouter is Maintainable {
    ...
    function unwrapAndFindBestPath(
        ...
        for (uint256 i; i < wrapperTokenOut.length; ++i) {
            uint256 amountOut = IWrapper(wrapper).query(
                amountIn,
                wrappedToken,
                wrapperTokenOut[i]
            );
            ...
            if (wrapperTokenOut[i] != tokenOut) {
                ...
                Offer memory query = OfferUtils.newOffer(
                    amountIn,
                    wrappedToken
                );
                offer = query.format();
